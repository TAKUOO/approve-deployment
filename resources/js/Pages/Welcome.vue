        ...
        <div class="flex relative z-10 flex-col min-h-screen">
            ...
            <main class="flex-1">
                    ...
                    <div class="mx-auto grid max-w-6xl gap-8 sm:gap-12 lg:grid-cols-[0.85fr_1.15fr] items-center">
                        <div class="space-y-4 text-center sm:space-y-8 lg:space-y-6 lg:text-left">
                            <div>
                                ...
                                <h1 class="text-3xl font-semibold tracking-tight leading-tight text-slate-700 sm:text-5xl md:text-5xl lg:text-4xl">
                                    <span class="block mb-2 sm:mb-3 lg:mb-4">WEBデザイナー向け</span>
                                    <span class="block break-words">リリース自動化システム</span>
                                </h1>
                            </div>

                            <!-- スマホ時のみ表示するイラスト（タイトルと説明文の間） -->
                            <div class="flex relative justify-center items-center lg:hidden">
                                <div class="relative w-full max-w-xs">
                                    <!-- 背景のグラデーションエフェクト -->
                                    <div class="absolute inset-0 rounded-[24px] bg-gradient-to-br from-indigo-200/40 via-purple-200/30 to-pink-100/30 blur-[60px]"></div>
                        ...
                        <!-- デスクトップ時のみ表示するイラスト（右側） -->
                        <div class="hidden relative justify-center items-center lg:flex lg:mt-0">
                            <div class="relative w-full max-w-xl lg:max-w-2xl xl:max-w-3xl">
                                <!-- 背景のグラデーションエフェクト -->
                ...
                <section id="features" class="relative px-4 py-12 sm:px-6 sm:py-16 lg:py-20">
                    <div class="relative mx-auto space-y-6 max-w-6xl sm:space-y-8 lg:space-y-10">
                        <div class="space-y-2 text-center sm:space-y-3">
                            <p class="text-xs uppercase tracking-[0.3em] text-indigo-500">Capabilities</p>
                            <h2 class="text-2xl font-semibold sm:text-3xl text-slate-700">更新時の二重作業を抑え、無駄な時間を排除</h2>
                            <p class="px-2 text-sm sm:text-base text-slate-600">AutoReleaseは、テスト→承認→本番アップを一本化し、同じ作業を繰り返さないための仕組みを標準搭載しています。</p>
                        </div>
                        <div class="grid gap-6 sm:gap-8 md:grid-cols-3">
                            ...
                            >
                                <div class="relative space-y-3 sm:space-y-4">
                                    ...
                                    <h3 class="text-xl font-semibold sm:text-2xl" :class="feature.textColor">{{ feature.title }}</h3>
                ...
                <section id="workflow" class="relative px-4 py-12 sm:px-6 sm:py-16 lg:py-20">
                    <div class="relative mx-auto grid max-w-6xl gap-6 sm:gap-8 lg:gap-10 rounded-[24px] sm:rounded-[32px] lg:rounded-[36px] border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-indigo-50/60 p-4 sm:p-6 lg:p-10 shadow-[0_35px_120px_rgba(79,70,229,0.15)] ring-1 ring-indigo-50 lg:grid-cols-[1.1fr_0.9fr]">
                        <div class="space-y-4 sm:space-y-6">
                            ...
                            <h2 class="text-2xl font-semibold sm:text-3xl text-slate-700">4ステップで承認〜自動デプロイ</h2>
                            ...
                            <div class="space-y-3 sm:space-y-4">
                                <div
                                    ...
                                    :key="step.title"
                                    class="flex gap-3 p-4 rounded-2xl border border-indigo-100 sm:gap-4 sm:p-5 to-indigo-100/90"
                                >
                                    <div class="flex flex-shrink-0 justify-center items-center w-10 h-10 text-xs font-semibold text-indigo-600 bg-indigo-100 rounded-2xl sm:w-12 sm:h-12 sm:text-sm">
                                        {{ step.title.split('.')[0] }}
                                    </div>
                                    <div>
                                        <p class="text-base font-semibold sm:text-lg text-slate-700">{{ step.title }}</p>
                                        <p class="mt-1 text-xs sm:mt-2 sm:text-sm text-slate-600">{{ step.detail }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="relative overflow-hidden rounded-[24px] sm:rounded-[32px] border border-indigo-100 bg-indigo-100 p-4 sm:p-6 lg:p-8 mt-6 lg:mt-0">
                            <div class="mb-4 space-y-2 sm:mb-6">
                                <p class="text-xs text-indigo-500 sm:text-sm">Live status</p>
                                <p class="text-xl font-semibold sm:text-2xl text-slate-700">AutoRelease Bot</p>
                                <p class="text-xs break-words sm:text-sm text-slate-500">github-actions · production</p>
                            </div>
                            <div class="space-y-4 text-xs sm:space-y-6 sm:text-sm text-slate-600">
                                <div class="p-4 bg-white rounded-2xl shadow-sm sm:p-5 sm:rounded-3xl">
                                    <p class="text-xs uppercase tracking-[0.3em] text-indigo-500">Command</p>
                                    <pre class="overflow-x-auto p-3 mt-2 text-xs text-emerald-200 rounded-xl sm:p-4 sm:mt-3 sm:text-sm sm:rounded-2xl bg-slate-900/90">workflow_dispatch --env=production --signed</pre>
                                    <p class="mt-2 text-xs break-words text-slate-500">最終承認者: Client Team · IP 203.0.113.4</p>
                                </div>
                                <div class="space-y-3 sm:space-y-4">
                                    ...
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span class="text-slate-700">00:45</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <section id="security" class="relative px-4 py-12 sm:px-6 sm:py-16 lg:py-20">
                    <div class="relative mx-auto space-y-6 max-w-6xl sm:space-y-8 lg:space-y-10">
                        <div class="text-center">
                            ...
                            <h2 class="mt-2 text-2xl font-semibold sm:text-3xl text-slate-700">セキュリティ対策</h2>
                            <p class="px-2 mt-3 text-sm sm:mt-4 sm:text-base text-slate-600">クライアントワークでも安心してご利用いただけるよう、多層的なセキュリティ対策を実装しています。</p>
                        </div>

                        <div class="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
                            <div class="p-4 bg-white rounded-2xl border shadow-sm sm:p-6 border-slate-200">
                                <div class="flex gap-2 items-center mb-3 sm:gap-3 sm:mb-4">
                                    <div class="flex flex-shrink-0 justify-center items-center w-8 h-8 bg-indigo-100 rounded-xl sm:w-10 sm:h-10">
                                        <svg class="w-5 h-5 text-indigo-600 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                    </div>
                                    <h3 class="text-base font-semibold sm:text-lg text-slate-700">有効期限付きトークン</h3>
                                </div>
                            ...
                            <div class="p-4 bg-white rounded-2xl border shadow-sm sm:p-6 border-slate-200">
                                <div class="flex gap-2 items-center mb-3 sm:gap-3 sm:mb-4">
                                    <div class="flex flex-shrink-0 justify-center items-center w-8 h-8 bg-green-100 rounded-xl sm:w-10 sm:h-10">
                                        <svg class="w-5 h-5 text-green-600 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                        </svg>
                                    </div>
                                    <h3 class="text-base font-semibold sm:text-lg text-slate-700">トークンベース認証</h3>
                                </div>
                            ...
                            <div class="p-4 bg-white rounded-2xl border shadow-sm sm:p-6 border-slate-200">
                                <div class="flex gap-2 items-center mb-3 sm:gap-3 sm:mb-4">
                                    <div class="flex flex-shrink-0 justify-center items-center w-8 h-8 bg-yellow-100 rounded-xl sm:w-10 sm:h-10">
                                        <svg class="w-5 h-5 text-yellow-600 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                        </svg>
                                    </div>
                                    <h3 class="text-base font-semibold sm:text-lg text-slate-700">レート制限</h3>
                                </div>
                            ...
                            <div class="p-4 bg-white rounded-2xl border shadow-sm sm:p-6 border-slate-200">
                                <div class="flex gap-2 items-center mb-3 sm:gap-3 sm:mb-4">
                                    <div class="flex flex-shrink-0 justify-center items-center w-8 h-8 bg-purple-100 rounded-xl sm:w-10 sm:h-10">
                                        <svg class="w-5 h-5 text-purple-600 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                        </svg>
                                    </div>
                                    <h3 class="text-base font-semibold sm:text-lg text-slate-700">XSS対策</h3>
                                </div>
                            ...
                            <div class="p-4 bg-white rounded-2xl border shadow-sm sm:p-6 border-slate-200">
                                <div class="flex gap-2 items-center mb-3 sm:gap-3 sm:mb-4">
                                    <div class="flex flex-shrink-0 justify-center items-center w-8 h-8 bg-blue-100 rounded-xl sm:w-10 sm:h-10">
                                        <svg class="w-5 h-5 text-blue-600 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                        </svg>
                                    </div>
                                    <h3 class="text-base font-semibold sm:text-lg text-slate-700">検索エンジン対策</h3>
                                </div>
                            ...
                            <div class="p-4 bg-white rounded-2xl border shadow-sm sm:p-6 border-slate-200">
                                <div class="flex gap-2 items-center mb-3 sm:gap-3 sm:mb-4">
                                    <div class="flex flex-shrink-0 justify-center items-center w-8 h-8 bg-red-100 rounded-xl sm:w-10 sm:h-10">
                                        <svg class="w-5 h-5 text-red-600 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                        </svg>
                                    </div>
                                    <h3 class="text-base font-semibold sm:text-lg text-slate-700">情報の非公開化</h3>
                                </div>
                                <p class="text-xs sm:text-sm text-slate-600">承認ページではGitHubリポジトリ情報などの機密情報を非表示にし、必要最小限の情報のみを表示しています。</p>
                            </div>
                        </div>

                        <div class="p-4 mt-6 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-200 sm:p-6 lg:p-8 sm:mt-8">
                            <div class="flex gap-3 items-start sm:gap-4">
                                <div class="flex-shrink-0">
                                    <svg class="w-6 h-6 text-indigo-600 sm:w-7 sm:h-7 lg:w-8 lg:h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                    </svg>
                                </div>
                                <div>
                                    <h3 class="mb-2 text-lg font-semibold sm:text-xl text-slate-700">クライアントワークでも安心</h3>
                ...
                <section id="faq" class="relative px-4 pb-12 sm:px-6 sm:pb-16 lg:pb-20">
                    <div class="relative mx-auto space-y-6 max-w-5xl sm:space-y-8 lg:space-y-10">
                        <div class="text-center">
                            <p class="text-xs uppercase tracking-[0.3em] text-indigo-500">FAQ</p>
                            <h2 class="mt-2 text-2xl font-semibold sm:text-3xl text-slate-700">よくある質問</h2>
                            <p class="px-2 mt-3 text-sm sm:mt-4 sm:text-base text-slate-600">導入前の疑問はすべて公開しています。ほかに質問があればSlackコミュニティへ。</p>
                        </div>
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            ...
                            >
                                <button
                                    ...
                                    class="flex gap-2 justify-between items-center px-4 py-4 w-full text-base font-semibold text-left sm:px-6 sm:py-5 sm:text-lg text-slate-700"
                                >
                                ...
                                <div v-show="openFaqIndex === index" class="px-4 pb-4 text-xs leading-relaxed sm:px-6 sm:pb-6 sm:text-sm text-slate-600">
                                    {{ faq.answer }}
                                </div>
                            </div>
                        </div>
                ...
                <section v-if="!$page.props.auth?.user" class="px-4 pb-16 sm:px-6 sm:pb-20 lg:pb-24">
                    <div class="mx-auto max-w-5xl rounded-[24px] sm:rounded-[32px] lg:rounded-[40px] border border-slate-200 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-1">
                        <div class="rounded-[20px] sm:rounded-[28px] lg:rounded-[36px] bg-gradient-to-br from-white via-slate-50 to-indigo-50/50 px-4 py-8 sm:px-6 sm:py-10 lg:px-10 lg:py-12 text-center text-slate-700 shadow-[0_25px_80px_rgba(79,70,229,0.15)]">
                            <p class="text-xs uppercase tracking-[0.3em] text-indigo-500">Get started</p>
                            <h2 class="mt-3 text-xl font-semibold sm:mt-4 sm:text-2xl lg:text-3xl">GitHubアカウントで今日から運用を自動化</h2>
                            <p class="mt-2 text-sm sm:mt-3 sm:text-base text-slate-600">ログインして承認URLを送り、ワンクリックでGitHub Actionsを呼び出すだけ。初期費用はゼロです。</p>
