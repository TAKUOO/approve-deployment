...
AutoReleaseは「相手との仕事を、ワンストップで"リリース"まで持っていく領域」に特化したプロダクトです。この一文を軸に、やること・やらないことが明確になっています。

### 領域定義（コア思想）

> **「相手との仕事を、ワンストップで"リリース"まで持っていく領域」**

この定義から逆算すると、AutoReleaseは以下**ではありません**。

- 雑談ツールではない
- 常時コミュニケーションの場ではない
- 何でも管理するプロジェクト管理ツールではない

### AutoReleaseが「やること」

| 役割 | 内容 | 目指す状態 |
|------|------|------------|
| **① リリースに向けた合意の可視化** | 何が終わっているか／何が残っているか／誰待ちで止まっているか | リリース可否を判断できる状態を作る |
| **② 「確認してください」を迷わせない** | クライアント待ちタスクの明確化、OGPで一瞬で伝わる、URL1つで状況共有 | “確認お願いします”の摩擦を最小化 |
| **③ 承認という「最後の一歩」を担う** | タスクが揃った・修正が終わった先の責任の線を引く | リリースという境界線を明確に引く |

### AutoReleaseが「やらないこと」

「リリースに関係ないもの」は潔く捨てる、という方針です。
...
### タスク管理を入れる理由（定義に収まる整理）

タスク管理を入れる理由は、**タスク = リリースに必要な“合意の単位”** だからです。

- **月間／週間／本日** → リリースまでの分解
- **自分タスク／相手タスク** → 止まっている責任の可視化
- **コメントは最小限** → 合意ログとして残す

いずれも「相手との仕事をワンストップでリリースする」という定義に収まります。

### OGP共有と定義の一致

OGPは「今リリースできる？」「どこが詰まってる？」を即答できる情報を出しています。  
→ **判断コストを下げる = リリースを早める**。名前「AutoRelease」とも噛み合っています。

### 今後の判断フィルター

新機能・アイデアが出たときの問いかけ：

> **「それは"相手との仕事をワンストップでリリースする"のに必要？」**
>
> - **YES** → やる  
> - **NO** → やらない or 別サービス

このフィルターでプロダクトのブレを防ぎます。

### まとめ：AutoReleaseの輪郭

| 担うもの | 担わないもの |
|----------|----------------|
| リリースまでの合意の整理 | 日常のやり取り（Slack） |
| 誰待ちかの明確化 | 感情のすり合わせ |
| 承認という境界線 | 社内向け細かい管理 |
| 進捗の一瞬共有（OGP） | 長期的な計画管理 |

**→ 「リリースに責任を持つための境界線ツール」** として設計されています。単なる自動デプロイ補助やWordPress向けツールではなく、クライアントワークの責任の線を明確にする道具です。

---

*次のステップ例: この領域定義を1行ミッションとして固定 → 機能一覧を「やる／やらない」に仕分け → LPのメッセージに落とす*
